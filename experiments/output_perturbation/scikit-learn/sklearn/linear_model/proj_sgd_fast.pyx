...
DEF PA1 = 5
DEF PA2 = 6
# For BOLT-ON projected SGD with strongly convex loss
DEF BOLTON = 7
...
def plain_proj_sgd(np.ndarray[double, ndim=1, mode='c'] weights,
              double intercept,
              LossFunction loss,
              int penalty_type,
    ...
    standard_weights, standard_intercept,\
        _, _, n_iter_ = _plain_proj_sgd(weights,
                                   intercept,
                                   None,
                                   0,
                                   loss,
...
def average_proj_sgd(np.ndarray[double, ndim=1, mode='c'] weights,
                double intercept,
    ...
    n_iter_ : int
        The actual number of iter (epochs).
    """
    return _plain_proj_sgd(weights,
                      intercept,
                      average_weights,
                      average_intercept,
                      loss,
...
def _plain_proj_sgd(np.ndarray[double, ndim=1, mode='c'] weights,
               double intercept,
               np.ndarray[double, ndim=1, mode='c'] average_weights,
               double average_intercept,
               LossFunction loss,
    ...
    if learning_rate == BOLTON:
        if penalty_type != L2 or not isinstance(loss, Log):
            print('[_plain_proj_sgd] Warning: BOLTON learning_rate only makes sense for Log loss with L2 penalty!')
        ...
        for epoch in range(max_iter):
            ...
            for i in range(n_samples):
                ...
                if learning_rate == OPTIMAL:
                    ...
                    eta = min(1.0/(1.0+alpha), 1.0/(alpha*t))
                ...
                # Project w back to the right L2 ball
                # For now, we ignore the intercept
                if radius > 0:
                    norm_w = w.norm()
                    if norm_w > radius:
                        w.scale(radius/norm_w)
